...
Провайдер сообщил о большом количестве исходящих соединений на TCP/25? Установите инструмент и запустите пошаговое read-only расследование:
...
Мастер спросит порт, период журналов и время наблюдения, а затем покажет краткую оценку и путь к приватному каталогу доказательств. Он не блокирует адреса, не останавливает службы и не меняет конфигурацию почты.
...
Версия **1.2.0**, лицензия MIT. Инструмент не меняет firewall, Postfix, SSH и Fail2ban, не блокирует IP и не записывает payload пакетов или тела писем.
...
## Интерактивное расследование жалобы провайдера

Если хостинг-провайдер сообщил об аномально большом количестве исходящих соединений на TCP/25, запустите компактный пошаговый режим:
...
Интерактивный мастер требует root-доступа, чтобы полностью читать журналы и связывать пакеты с PID. Он спрашивает только SMTP-порт назначения, период журналов и длительность ограниченного наблюдения. После этого он:

1. сохраняет изменчивые данные о сокетах и исходящих SYN;
2. собирает контекст Postfix, очереди, входов, заданий и контейнеров;
3. показывает краткую осторожную оценку и путь к приватному отчёту.

Оценка различает трафик известного MTA, возможные ретраи Postfix, соединения процесса вне известных MTA, трафик без установленного владельца, событие, не воспроизведённое при работающем захвате, и неполные данные. Отсутствие соединений в коротком окне никогда не объявляется доказательством безопасности сервера.

Если в journald нет подходящих записей и используется обычный файл журнала, итог помечает источник как `unbounded-file`. Эти счётчики остаются в отчёте для контекста, но не используются как доказательство ретраев в выбранном временном окне.

Подробный технический вывод сохраняется в приватных файлах каталога запуска и не перегружает терминал. Даже при пропуске live-захвата сохраняется мгновенный снимок сокетов. Ctrl+C досрочно завершает наблюдение и продолжает формирование отчёта. Мастер работает только в настоящем терминале и остаётся полностью read-only; для автоматизации и systemd используйте явные команды ниже.
...
- `interactive` — пошаговое и компактное расследование жалобы провайдера; также запускается автоматически без аргументов в терминале;
